...
app.post('/token', async (c) => {
  ...
  const expectedHashBuffer = await crypto.subtle.digest('SHA-256', new TextEncoder().encode(code_verifier as string));
  const expectedHash = btoa(String.fromCharCode(...new Uint8Array(expectedHashBuffer))).replace(/\+/g, '-').replace(/\//g, '_').replace(/=+$/, '');
  if (expectedHash !== storedData.code_challenge) {
    return c.text('Invalid code verifier', 400);
  }

  const privateKey = await jose.importPKCS8(c.env.PRIVATE_KEY, 'RS256');
...
const verifyToken = async (c: Context, next: Next) => {
  ...
  try {
    const publicKey = await jose.importSPKI(c.env.PUBLIC_KEY.replace(/\\n/g, '\n'), 'RS256');
